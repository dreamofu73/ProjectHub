# Frontend 개발 표준 가드레일 (apps/web/CLAUDE.md)

이 문서는 **React Vite (TypeScript) 프런트엔드** 소스 코드의 품질, 컴파일 안정성 및 백엔드와의 매끄러운 통합 배포를 유지하기 위한 가이드라인입니다.

분야별 구체적인 개발 가이드는 아래 링크를 참조하십시오:

- [01. 아키텍처 및 라우팅 규칙](docs/guides/01_architecture_and_routing.md)
- [02. UI 개발 워크플로우 및 스타일 규칙](docs/guides/02_ui_development_workflow.md)
- [03. 다국어(i18n) 지원 규칙](docs/guides/03_i18n_rules.md)
- [04. 화면 패턴 규칙](docs/guides/04_screen_patterns.md)
- [05. 컴파일, 빌드 및 테스트 규칙](docs/guides/05_build_and_test.md)
- [06. 타이포그래피 규칙](docs/guides/06_typography_rules.md)

---

> [!IMPORTANT]
> 모든 개발자는 위 가이드라인을 준수해야 하며, 특히 **모달 다이얼로그 사용 금지**, **`./scripts/web/dev.sh` 실행** 등의 제한 사항을 반드시 숙지하십시오.
