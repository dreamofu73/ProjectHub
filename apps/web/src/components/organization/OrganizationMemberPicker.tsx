...
export interface OrganizationMemberPickerProps {
  title?: ReactNode;
  titleInput?: {
    value: string;
  ...
  onSave: (finalIds: string[], addedIds: string[], removedIds: string[]) => Promise<void>;
  onClose: () => void;
  saving?: boolean;
  t: (key: string) => string;
  
  footerLeft?: ReactNode; // E.g., for Role selector
}
...
export function OrganizationMemberPicker({
  ...
  const handleSave = async () => {
    const finalIds = Array.from(memberUserIds);
    const initialSet = new Set(initialMemberIds);
    const addedIds = finalIds.filter(id => !initialSet.has(id));
    const removedIds = initialMemberIds.filter(id => !memberUserIds.has(id));
    
    await onSave(finalIds, addedIds, removedIds);
  };
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)] shrink-0">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          {titleInput ? (
            ...
            <div className="flex-1 min-w-0 text-base font-bold text-[var(--text-primary)]">
              {title}
            </div>
          )}
        </div>
      ...
      {/* ── Footer ── */}
      <div className="flex items-center justify-between px-6 py-3 border-t border-[var(--border)] shrink-0 bg-[var(--bg-surface-2)]/20">
        <div>
          {footerLeft}
        </div>
        <div className="flex items-center gap-2">
