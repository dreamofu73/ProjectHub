    ...
    <div className="flex flex-col flex-1 overflow-hidden bg-[var(--bg-surface)]">
      ...
      <form onSubmit={handleSendMemo} className="flex-1 overflow-y-auto flex flex-col p-6 space-y-5 custom-scrollbar">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-[var(--text-secondary)]">제목 <span className="text-red-500">*</span></label>
          <input type="text" placeholder="쪽지 제목을 입력하세요" value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full px-3.5 py-2 h-9.5 border border-[var(--border)] rounded-xl bg-[var(--bg-surface)] text-xs focus:outline-none focus:ring-1 focus:ring-[var(--primary)]/50 focus:border-[var(--primary)] transition-all placeholder:text-[var(--text-muted)] text-[var(--text-primary)]" />
        </div>
            ...
            <>
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <input type="text" placeholder="이름 또는 아이디 검색하여 추가..." value={recipientSearch} onChange={(e) => setRecipientSearch(e.target.value)} className="w-full px-3.5 py-2 h-9.5 border border-[var(--border)] rounded-xl bg-[var(--bg-surface)] text-xs focus:outline-none focus:ring-1 focus:ring-[var(--primary)]/50 focus:border-[var(--primary)] transition-all placeholder:text-[var(--text-muted)] text-[var(--text-primary)]" />
                ...
                >
                  <Users size={13} />
                  <span className="hidden sm:inline">수신그룹</span>
                </button>
              </div>
              {recipientSearch.trim() !== '' && (
                <div className="absolute left-0 right-0 mt-1 max-h-48 overflow-y-auto bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl shadow-xl z-35 divide-y divide-[var(--border)] animate-in fade-in slide-in-from-top-1 duration-150 custom-scrollbar" style={{ top: '100%' }}>
                  {filteredUsers.length === 0 ? (
                    <div className="p-3 text-center text-xs text-[var(--text-muted)] font-medium">검색 결과가 없거나 이미 추가되었습니다.</div>
                  ) : (
                    filteredUsers.map((u) => (
        ...
        <div className="space-y-2 p-4 bg-[var(--bg-surface-2)] border border-[var(--border)] rounded-2xl">
          ...
          {isReservedSend && (
            <div className="mt-2.5 flex items-center gap-2.5 animate-in fade-in duration-200">
              ...
              />
              <span className="text-xs text-[var(--text-muted)] font-medium">지정한 시간에 자동으로 발송됩니다.</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-[var(--text-secondary)] flex items-center gap-1">
            <Paperclip size={12} className="text-[var(--text-muted)]" />파일 첨부
          </label>
        ...
        <div className="flex items-center justify-end gap-2 pt-5 border-t border-[var(--border)] shrink-0">
          <Button type="button" onClick={() => setIsComposeOpen(false)} className="bg-[var(--bg-surface-2)] hover:opacity-90 text-[var(--text-secondary)] font-bold px-4 py-2 rounded-xl text-xs border-none cursor-pointer h-9">취소</Button>
          <Button type="submit" disabled={sending} className="bg-[var(--primary)] hover:opacity-90 text-white font-bold px-5 py-2 rounded-xl text-xs flex items-center gap-1.5 cursor-pointer disabled:opacity-50 h-9 border-none">
            {sending ? <><RefreshCw size={13} className="animate-spin" />전송 중...</> : <><Send size={13} />전송하기</>}
          </Button>
        </div>
      </form>
    </div>
