...
export function Sidebar() {
  ...
  return (
    <aside className="sidebar relative overflow-visible h-[calc(100vh-var(--header-height))] sticky top-0 z-40 border-r border-slate-200 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/30" aria-label={t('sidebar')}>
              ...
              <ul className="sidebar-nav" aria-label={t('chatChannelList')}>
                ...
                {chatRooms.length === 0 && !isSidebarCollapsed && (
                  <div className="flex flex-col items-center text-center py-8 px-3 gap-2">
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-500">{t('chatNoProjectChannels')}</p>
                    <button
                      onClick={() => window.dispatchEvent(new CustomEvent('open_create_chat_room'))}
