  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setError(json.error || t('failToLoadProjects') || 'Failed to load projects.');
      }
    } catch {
      setError(t('serverConnectionError') || 'Server connection error occurred.');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader 
        ...
        description={t('projectsPageDesc') || 'List of participating or public projects.'}
        actions={
          <Link to="/projects/new">
            <Button icon={Plus}>{t('newProject') || 'New Project'}</Button>
          </Link>
        }
      />
          ...
          <CardBody className="p-1.5 flex flex-row gap-2 items-center">
            <form onSubmit={handleSearchSubmit} className="flex flex-wrap md:flex-nowrap items-center gap-2">
              ...
              <div className="w-64">
                <Input 
                  icon={Search}
                  placeholder={t('searchProjectsPlaceholder') || 'Search project name, description or identifier'} 
                  value={searchTerm} 
              ...
              <Button type="submit" variant="primary" size="sm" className="shrink-0">{t('search') || 'Search'}</Button>
            </form>
          </CardBody>
        </Card>
      </div>
      ...
      {loading ? (
        ...
        <Card className="border-danger">
          <CardBody className="text-center py-12">
            ...
            <Button variant="secondary" className="mt-4" onClick={fetchProjects}>{t('retry') || 'Retry'}</Button>
          </CardBody>
        </Card>
      ) : projects.length === 0 ? (
        <Card>
          <CardBody className="text-center py-20 text-muted">
            ...
            <p>{t('noProjects') || 'No projects exist.'}</p>
          </CardBody>
        </Card>
      ) : viewMode === 'card' ? (
              ...
              <Card key={project.id} className="group relative overflow-hidden border border-gray-200 dark:border-slate-700/60 hover:border-primary/50 hover:shadow-lg transition-all duration-300 bg-white dark:bg-slate-900 rounded-xl">
                ...
                <div className="p-5 pb-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {project.is_public === 1 ? (
                        <Badge variant="info" icon={<Globe size={10} />}>{t('public') || 'Public'}</Badge>
                      ) : (
                        <Badge variant="default" icon={<Lock size={10} />}>{t('private') || 'Private'}</Badge>
                      )}
                    </div>
                  </div>

                  <p className="mt-3 text-sm text-secondary dark:text-slate-400 line-clamp-2 leading-relaxed min-h-[2.5em]">
                    {project.description || t('noDescriptionRegistered') || 'No project description.'}
                  </p>
                </div>
                ...
                <div className="px-5 pb-3">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[11px] font-bold text-muted dark:text-slate-500 uppercase tracking-wider">{t('progress') || 'Progress'}</span>
                ...
                <div className="border-t border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/30 px-5 py-3 flex items-center justify-between">
                  <div className="flex gap-1">
                    ...
                    >
                      <Users size={13} />
                      {t('members') || 'Members'} <span className="text-gray-500 dark:text-slate-400 font-normal">{project.member_count}</span>
                    </Link>
                    <Link
                    ...
                    >
                      <Bug size={13} />
                      {t('issues') || 'Issues'} <span className="text-gray-500 dark:text-slate-400 font-normal">{project.open_issue_count}</span>
                    </Link>
                  </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 dark:bg-slate-800/50 border-b border-border">
                  <th className="py-3 px-4 text-xs font-semibold text-muted uppercase tracking-wider">{t('projects') || 'Projects'}</th>
                  <th className="py-3 px-4 text-xs font-semibold text-muted uppercase tracking-wider">{t('visibility') || 'Visibility'}</th>
                  <th className="py-3 px-4 text-xs font-semibold text-muted uppercase tracking-wider">{t('progress') || 'Progress'}</th>
                  <th className="py-3 px-4 text-xs font-semibold text-muted uppercase tracking-wider">{t('members') || 'Members'}</th>
                  <th className="py-3 px-4 text-xs font-semibold text-muted uppercase tracking-wider">{t('issues') || 'Issues'}</th>
                  <th className="py-3 px-4 text-xs font-semibold text-muted uppercase tracking-wider text-right">{t('details') || 'Details'}</th>
                </tr>
              </thead>
