  ...
  const handleComposeSendMemo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        showToast(composeIsReservedSend ? t('memoScheduled') : t('memoSendSuccess'), 'success');
        ...
        showToast(json.error || t('memoSendFail'), 'error');
      }
    } catch (err) {
      console.error(err);
            ...
            <div className="px-6 py-3 border-b border-[var(--border)] bg-[var(--bg-surface-2)]/30 animate-in slide-in-from-top-2 duration-200">
              <div className="flex gap-2 items-center">
                ...
                <Button onClick={showGroupModal === 'create' ? handleCreateGroup : handleUpdateGroup} size="sm">
                  {showGroupModal === 'create' ? t('create') : t('save')}
                </Button>
                <Button variant="secondary" size="sm" onClick={() => setShowGroupModal(null)}>{t('cancel')}</Button>
              </div>
            </div>
          )}
